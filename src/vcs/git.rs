...
impl Git {
    ...
    fn git_status(&self) -> Result<Status> {
        let path_str = self
            .root_dir
            .to_str()
...
///  M src/main.rs
/// ```
fn parse_porcelain_output<S: Into<String>>(porcelain: S) -> Result<Status> {
    let porcelain_str = porcelain.into();
    let porcelain_lines = porcelain_str.lines();
    let mut vcs_status: Status = Default::default();
...
fn increment_vcs_status(vcs_status: &mut Status, letter: char) {
    match letter {
...
mod tests {
    ...
    fn test_parse_empty_porcelain_output() -> Result<()> {
        ...
        let expected: Status = Default::default();
